...
#	app package downloads, a copy (linked file) of the associated installer package will be renamed and moved to your desktop.


TEMP_PATH=`/usr/bin/mktemp -d /tmp/MAS_XXXX`
SEARCHPATH="$(/usr/bin/getconf DARWIN_USER_CACHE_DIR)/com.apple.appstore"
...
while $FOUND; do
    for FILE in $PACKAGELIST; do
        if [ ! -e "${FILE}" ]; then
            TITLE=`/usr/sbin/installer -verbose -pkginfo -pkg "${TEMP_PATH}/${FILE##*/}" | /usr/bin/grep -m 1 Title | /usr/bin/awk -F " : " '{print $2}'`
            /bin/mv "${TEMP_PATH}/${FILE##*/}" "$HOME/Desktop/$TITLE.pkg"
            FOUND=false
        fi
    done
done
